...
uint8_t runCommand(commandTemplate* current_command){
    ...
    else if (strcmp(current_command->command,"EXT") == 0){
        ExtPortsParser(current_command);
    }

    else if (strcmp(current_command->command,"OPT_INPUTS") == 0){
        OptInputsParser(current_command);
    }
